...
class UserController extends Controller
{
    ...
    public function editprofile(Request $request)
    {
        ...
        if ($request->hasFile('profile')) {
            ...
            $maxFileSizeKB = 500; // Maximum file size in KB (500 KB in this case)
            $fileSizeKB = $file->getSize() / 1024; // Convert file size to KB
            $extension       = $request->file('profile')->getClientOriginalExtension();
           

            if (strtolower($extension) !== 'jpeg') {
            return redirect()->route('profile', \Auth::user()->id)->with('error', __('The file must be a JPG image.'));
            }
            
            if ($fileSizeKB > $maxFileSizeKB) {
            return redirect()->route('profile', \Auth::user()->id)->with('error', __('The file size must be less than ' . $maxFileSizeKB . ' KB.'));
            }
        ...
        if (\Auth::Check()) {
            ...
            if (Hash::check($request_data['current_password'], $current_password)) {
                ...
                if ($superAdmin) {
                    ...
                    Mail::send('email.password_change_notification', $data, function ($message) use ($superAdmin) {
                        $message->to($superAdmin->email)
                            ->subject('User Password Change Notification');
                    });
                }
