...
use App\Models\EmployeeJourney;
use App\Models\Increment;
use App\Models\IncrementJourney;
...
class EmployeeController extends Controller
{
    ...
    public function employee_increment()
    {
        ...
        if (\Auth::user()->can('Manage Employee')) {
            $increments = Increment::with(['employee'])->get();
            $employees = Employee::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
            return view('employee.increment', compact('increments', 'employees'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    public function employee_increment_create(Request $request)
    {
        if (\Auth::user()->can('Manage Employee')) {
            $increments = Increment::all();
            $employees = Employee::where('created_by', \Auth::user()->creatorId())->get()->pluck('name', 'id');
            return view('employee.incrementCreate', compact('increments', 'employees'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    
    public function employee_increment_store(Request $request)
    {
        if (\Auth::user()->can('Manage Employee')) {
            $rules = [
            ...
            $increment->user_id = $user->id;
            $increment->year = $request->year;
            $increment->salary = $request->salary;
            $increment->percentage = $request->percentage;
            $increment->save();
            $journey = new IncrementJourney();
            $journey->employee_id = $increment->employee_id;
            $journey->user_id = $increment->user_id;
            $journey->year = $increment->year;
            $journey->salary = $increment->salary;
            $journey->percentage = $increment->percentage;
            $journey->increment_id = $increment->id;
            $journey->save();
            return redirect()->route('employee_increment')->with('success', __('Increment created successfully.'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    
     public function employee_increment_edit($id)
    {
        $increment = Increment::findOrFail($id);
        $employees = Employee::where('created_by', \Auth::user()->creatorId())->pluck('name', 'id');
        return view('employee.increment_edit', compact('increment', 'employees'));
    }

    public function employee_increment_update(Request $request, $id)
    {
        $request->validate([
        ...
        return redirect()
            ->route('employee_increment')
            ->with('success', __('Increment updated successfully.'));
    }


    public function employee_increment_delete($id)
    {
        $increment = Increment::findOrFail($id);
        IncrementJourney::where('increment_id', $increment->id)->delete();
        $increment->delete();
        return redirect()->back()->with('success', 'Employee increment deleted successfully.');
    }
        ...
        if (\Auth::user()->can('Create Employee')) {
            ...
            $employee = Employee::create(
                [
                    ...
                    'email' => $request['email'],
                    'add_details' => implode(',', $request['add_details']),
                    'password' => Hash::make($request['password']),
                    ...
                    'diver_license_exp_date' => $request['diver_license_exp_date'],
                    'onboarding_costing' => $request['onboarding_costing'],
                    'created_by' => \Auth::user()->creatorId(),
                ]
            );
    ...
    public function update(Request $request, $id)
    {
        ...
        if (\Auth::user()->can('Edit Employee')) {
            ...
            $employeeJourney = new EmployeeJourney();
            $employeeJourney->employees_journey_id = $employee->id;
            $employeeJourney->user_id = $employee->user_id;
            $employeeJourney->manager     = $employee->manager;
            $employeeJourney->increments     = $employee->increments;
            $employeeJourney->name        = $employee->name;
            $employeeJourney->dob         = $employee->dob;
            $employeeJourney->age         = $employee->age;
            $employeeJourney->grading     = $employee->grading;
            $employeeJourney->gender      = $employee->gender;
            $employeeJourney->phone       = $employee->phone;
            $employeeJourney->address     = $employee->address;
            $employeeJourney->email     = $employee->email;
            $employeeJourney->employee_id     = $employee->employee_id;
            $employeeJourney->branch_id     = $employee->branch_id;
            $employeeJourney->department_id     = $employee->department_id;
            $employeeJourney->designation_id     = $employee->designation_id;
            $employeeJourney->company_doj     = $employee->company_doj;
            $employeeJourney->documents   = $employee->documents;
            $employeeJourney->salary     = $employee->salary;
            $employeeJourney->created_by  = Auth::id();
            $employeeJourney->updated_at  = now();
            $employeeJourney->save();
            if ($request->salary) {
     ...
     public function destroy($id)
    {
        if (Auth::user()->can('Delete Employee')) {
            ...
            $ContractEmployee = Contract::where('employee_name', '=', $employee->user_id)->get();
            $payslips = PaySlip::where('employee_id', $id)->get();
            $employeeJourneys = EmployeeJourney::where('employee_id', $employee->id)->get();
            $IncrementJourney = IncrementJourney::where('employee_id', $employee->id)->get();
            $Increment = Increment::where('employee_id', $employee->id)->get();
            $employee->delete();
            $user->delete();
            ...
            foreach ($employeeJourneys as $journey) {
                $journey->delete();
            }
            foreach ($Increment as $value) {
                $value->delete();
            }
            foreach ($IncrementJourney as $IncrementJourneys) {
                $IncrementJourneys->delete();
            }
    ...
    public function import(Request $request)
    {
        ...
        for ($i = 1; $i <= count($employees) - 1; $i++) {
            ...
            $employeeData->iban_number          = $employee[18];
            $employeeData->passport_number     = $employee[19];
            $employeeData->passport_exp_date  = $employee[20];
            $employeeData->visa_no            = $employee[21];
            $employeeData->visa_exp_date     = $employee[22];
            $employeeData->diver_license_exp_date = $employee[23];
            $employeeData->onboarding_costing = $employee[24];
    ...
    public function lastlogin_download(Request $request)
    {
        ...
        // Filter the user details based on the month or employee selection
        $userdetailsQuery = DB::table('login_details')
            ->join('users', 'login_details.user_id', '=', 'users.id')
            ->select(DB::raw('login_details.*, users.id as user_id, users.name as user_name, users.email as user_email, users.type as user_type'))
            ->where(['login_details.created_by' => \Auth::user()->creatorId()]);

        // If the month filter is set
        if (!empty($request->month)) {
            $userdetailsQuery->where('date', '>=', $firstDayofMonth);
            $userdetailsQuery->where('date', '<=', $lastDayofMonth);
        } else {
            $userdetailsQuery->whereMonth('date', date('m'))->whereYear('date', date('Y'));
        }

        // If the employee filter is set
        if (!empty($request->employee)) {
            $userdetailsQuery->where('user_id', $request->employee);
        }
        ...
        // Prepare CSV data
        $csvData = [];
        $csvData[] = ['User ID', 'Name', 'Email', 'Type', 'Last Login Date'];
        ...
        foreach ($userdetails as $detail) {
            $csvData[] = [
                ...
                $detail->user_email,
                $detail->user_type,
                $detail->date,  // assuming 'date' field is the last login
            ];
        }

        // Convert array to CSV format
        $filename = 'employees_last_login_' . now()->format('Ymd_His') . '.csv';
