...
class ReportController extends Controller
{
    ...
    public function journey(Request $request)
    {
        if (\Auth::user()->can('Manage Report')) {
            ...
            $branch = Branch::where('created_by', $creatorId)->pluck('name', 'id')->prepend('All', '');
            $department = Department::where('created_by', $creatorId)->pluck('name', 'id')->prepend('All', '');
            $employeeList = Employee::where('created_by', $creatorId)->pluck('name', 'id')->prepend('All', '');
            ...
            foreach ($employees as $employee) {
                $increments = $employee->increments()->orderBy('year', 'asc')->get();
                $yearlyData = [];
                ...
                foreach ($increments as $increment) {
                    ...
                    $labels[] = $increment->year;
                }

                $data[] = [
                    'name' => $employee->name,
                    'manager' => $employee->manager,
                    'increments' => $yearlyData
                ];
            }
            ...
            return view('report.journey', compact('branch', 'department', 'employeeList'))
                ->with('data', json_encode($data))
                ->with('labels', json_encode(array_values(array_unique($labels))))
    ...
    public function getsingleEmployees(Request $request)
    {
        if (\Auth::user()->can('Manage Report')) {
            $creatorId = \Auth::user()->creatorId();
            ...
            $branch = Branch::where('created_by', $creatorId)->where('id', $branchId)->first();
            $department = Department::where('created_by', $creatorId)->where('id', $departmentId)->first();
            $employee = EmployeeJourney::where('employees_journey_id', $employeeId)->with('designation')->get();
            
             if (!$branchId || !$departmentId || !$employeeId) {
            ...
            $journeyEvents = [
                [
                    ...
                    'description' => 'Employee joined the company on ' . ($employee->first()->company_doj ?? 'N/A'),
                ],
                [
                    'icon' => 'fa-solid fa-building',
                    'title' => 'Branch Assignment',
                    'description' => 'Assigned to branch: ' . ($branch->first()->name ?? 'N/A'),
                ],
                [
                    'icon' => 'fa-solid fa-sitemap',
                    'title' => 'Department Assignment',
                    'description' => 'Assigned to department: ' . ($department->first()->name ?? 'N/A'),
                ],
                [
                ...
                [
                    ...
                    'title' => 'Award',
                    'description' => 'Received Best Employee of the Month',
                ],
            ];


            return view('report.singleemployeelist', [
                'employee' => $employee,
